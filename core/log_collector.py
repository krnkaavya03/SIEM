...
class LogCollector:
    ...
    # -----------------------------
    # Windows Event Log channel names — never a Linux path
    WINDOWS_CHANNELS = {"security", "system", "application",
                        "setup", "forwardedevents", "hardwareevents"}

    def _is_windows_source(self) -> bool:
        """
        Return True when the log source looks like a Windows Event Log channel
        rather than a Linux file path.
        Triggers on:
          - Known channel names  (Security, System, Application, ...)
          - Paths ending in .evtx
          - Windows-style drive paths  (C:\\...)
        """
        p = (self.remote_log_path or "").strip()
        if not p:
            return False
        if p.lower() in self.WINDOWS_CHANNELS:
            return True
        if p.endswith(".evtx"):
            return True
        if len(p) >= 2 and p[1] == ":":
    ...
    def collect_logs(self):
        ...
        if self.remote_server:
            is_win = self._is_windows_source()

            if is_win:
                # Windows Event Log via WinRM
                if winrm:
                    return self._collect_remote_windows_logs()
                elif paramiko:
                    print("[LOG COLLECTOR] pywinrm not installed — falling back to SSH + PowerShell.")
                    return self._collect_remote_windows_via_ssh()
                else:
                    print("[LOG COLLECTOR] Windows remote needs pywinrm or paramiko.")
                    return []
            else:
                # Linux log file via SSH
                if paramiko:
                    return self._collect_remote_linux_logs_ssh()
                else:
                    print("[LOG COLLECTOR] Linux SSH needs paramiko. Run: pip install paramiko")
                    return []
    ...
    def _collect_remote_windows_via_ssh(self):
        """
        Fallback: collect Windows Event Log via SSH + PowerShell when pywinrm
        is not installed. Requires OpenSSH server running on the Windows target.
        """
        channel = self.remote_log_path.strip() if self.remote_log_path else "Security"
        if channel.lower() not in self.WINDOWS_CHANNELS and not channel.endswith(".evtx"):
            channel = "Security"

        EVENT_IDS = "4624,4625,4634,4647,4648,4672,4673,4688,4698"
        ps_cmd = (
            f"Get-WinEvent -LogName {channel} -MaxEvents 200 "
            f"| Where-Object {{$_.Id -in {EVENT_IDS}}} "
            f"| Select-Object TimeCreated,Id,"
            f"@{{N='User';E={{$_.Properties[5].Value}}}},"
            f"@{{N='IP';E={{$_.Properties[18].Value}}}},"
            f"Message | ConvertTo-Json -Compress"
        )
        cmd = f'powershell -NonInteractive -Command "{ps_cmd}"'

        collected_logs = []
        try:
            client = paramiko.SSHClient()
            client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            client.connect(self.remote_server, username=self.username,
            ...
            if err:
                print(f"[LOG COLLECTOR] SSH/PS stderr: {err[:200]}")
            if not out:
                return []

            import json as _json
            logs = _json.loads(out)
            ...
            for log in logs:
                try:
                    timestamp = datetime.strptime(
                        str(log.get("TimeCreated", ""))[:19], "%Y-%m-%d %H:%M:%S"
                    )
                except Exception:
                ...
                collected_logs.append({
                    ...
                    "raw_message":   str(log.get("Message", ""))[:300],
                    "hour":          timestamp.hour,
                })
        except Exception as e:
            print(f"[LOG COLLECTOR] SSH+PowerShell Windows collection failed: {e}")

        print(f"[LOG COLLECTOR] Collected {len(collected_logs)} events via SSH+PowerShell from {self.remote_server} ({channel})")
        return collected_logs
    ...
    def _collect_remote_windows_logs(self):
        ...
        # PowerShell: formats timestamp as ISO string (avoids /Date()/ WinRM bug),
        # safely extracts username and IP with null guards per event ID.
        ps_script = f"""
...
$result = $events | ForEach-Object {{
    ...
    $user = 'N/A'
    $ip   = 'N/A'
    if ($_.Id -in @(4624,4625,4634,4648)) {{
        if ($props.Count -gt 5  -and $props[5].Value)  {{ $user = [string]$props[5].Value }}
        if ($props.Count -gt 18 -and $props[18].Value -and $props[18].Value -ne '-') {{ $ip = [string]$props[18].Value }}
    }} elseif ($_.Id -in @(4672,4673)) {{
        if ($props.Count -gt 1 -and $props[1].Value)   {{ $user = [string]$props[1].Value }}
    }} elseif ($props.Count -gt 0 -and $props[0].Value) {{
        $user = [string]$props[0].Value
    }}
    [PSCustomObject]@{{
...
"""
        ...
        try:
            ...
            if stderr:
                print(f"[LOG COLLECTOR] WinRM stderr: {stderr[:400]}")
            if not stdout or stdout.strip() == '[]':
                print(f"[LOG COLLECTOR] WinRM returned empty output for channel '{channel}'. "
                      "Check: account has permission to read event logs, "
                      "and channel name is correct (Security / System / Application).")
                self.raw_logs = []
                return []

            logs = _json.loads(stdout)
            if isinstance(logs, dict):
                logs = [logs]
            ...
            for log in logs:
                try:
                    timestamp = datetime.strptime(
                        str(log.get('TimeStr', ''))[:19], "%Y-%m-%d %H:%M:%S"
                    )
                except Exception:
                    timestamp = datetime.now()

                event_id   = int(log.get('Id', 0) or 0)
                event_type = self._map_windows_event_id(event_id)
                username   = str(log.get('User', 'N/A') or 'N/A').strip() or 'N/A'
                ip_address = str(log.get('IP',   'N/A') or 'N/A').strip()
                if ip_address in ('-', '::1', '', 'None', 'N/A'):
                    ip_address = 'N/A'
                ...
                collected_logs.append({
                    ...
                    'username':      username,
                    'ip_address':    ip_address,
                    'raw_message':   str(log.get('Msg', ''))[:300],
                    'hour':          timestamp.hour,
                })

        except _json.JSONDecodeError as e:
            print(f"[LOG COLLECTOR] JSON parse failed. Raw output snippet: {stdout[:300]}")
            print(f"[LOG COLLECTOR] JSON error: {e}")
        except Exception as e:
            print(f"[LOG COLLECTOR] WinRM collection failed: {e}")
            import traceback; traceback.print_exc()

        self.raw_logs = collected_logs
        print(f"[LOG COLLECTOR] Collected {len(collected_logs)} logs from remote Windows server ({channel})")
        return collected_logs
